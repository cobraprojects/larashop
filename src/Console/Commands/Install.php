...
class Install extends Command
{
    ...
    protected function runMigration()
    {
        ...
        if (!$this->permissionExists('LarashopCity')) {
            Artisan::call('multiauth:permissions LarashopCity');
            $this->info(Artisan::output());
        }

        if (!$this->permissionExists('LarashopOrder')) {
            Artisan::call('multiauth:permissions LarashopOrder');
            $this->info(Artisan::output());
        }

        if (!$this->permissionExists('LarashopSetting')) {
            Artisan::call('multiauth:permissions LarashopSetting');
            $this->info(Artisan::output());
        }

        if (!$this->permissionExists('LarashopSocial')) {
            Artisan::call('multiauth:permissions LarashopSocial');
            $this->info(Artisan::output());
        }

        if (!$this->permissionExists('LarashopPage')) {
            Artisan::call('multiauth:permissions LarashopPage');
            $this->info(Artisan::output());
        }
