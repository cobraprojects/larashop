    ...
    <div class="row mt-4">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route(LaraShop::adminName().'.category.update', $category->id) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="parent_id" name="parent_id">
                                ...
                                {{ Larashop::getCategoriesSelect(old('parent_id', $category->parent_id)) }}
                            </select>
                            @error('parent_id')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <textarea type="text" class="editor form-control" id="description" name="description">{{ old('description', $category->description) }}</textarea>
                        </div>

                        <div class="form-group">
                            <p>{{ $category->getFirstMedia('image')?$category->getFirstMedia('image')('thumb')->attributes(['class'=>'img-fluid img-thumbnail']):'' }}
                            </p>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="order" name="order" value="{{ old('order', $category->order) }}" />
                            @error('order')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                        </div>
