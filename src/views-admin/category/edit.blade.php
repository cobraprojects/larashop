...
<span class="breadcrumb-item active">تعديل قسم</span>
@endsection
    ...
    <div class="row mt-4">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route(LaraShop::adminName().'.category.update', $larashopCategory->id) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label for="name" class="form-control-label">اسم القسم <span class="text-danger">*</span> </label>
                            <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $larashopCategory->name) }}" />
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="slug" name="slug" value="{{ old('slug', $larashopCategory->slug) }}" readonly />
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="parent_id" name="parent_id">
                                ...
                                {{ Larashop::getCategoriesSelect(old('parent_id', $larashopCategory->parent_id)) }}
                            </select>
                            @error('parent_id')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <textarea type="text" class="editor form-control" id="description"
                                name="description">{{ old('description', $larashopCategory->description) }}</textarea>
                        </div>

                        <div class="form-group">
                            <p>{{ $larashopCategory->getFirstMedia('image')?$larashopCategory->getFirstMedia('image')('thumb')->attributes(['class'=>'img-fluid img-thumbnail']):'' }}
                            </p>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="order" name="order" value="{{ old('order', $larashopCategory->order) }}" />
                            @error('order')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                        </div>
