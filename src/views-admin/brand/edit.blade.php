    ...
    <div class="row mt-4">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route(LaraShop::adminName().'.brand.update', $larashopBrand->id) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label for="name" class="form-control-label">اسم البراند <span class="text-danger">*</span> </label>
                            <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $larashopBrand->name) }}" />
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="slug" name="slug" value="{{ old('slug', $larashopBrand->slug) }}" readonly />
                        ...
                        <div class="form-group">
                            <label for="description" class="form-control-label">وصف البراند</label>
                            <textarea type="text" class="editor form-control" id="description" name="description">{{ old('description', $larashopBrand->description) }}</textarea>
                        </div>

                        <div class="form-group">
                            <p>{{ $larashopBrand->getFirstMedia('image')?$larashopBrand->getFirstMedia('image')('thumb')->attributes(['class'=>'img-fluid img-thumbnail']):'' }}
                            </p>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="order" name="order" value="{{ old('order', $larashopBrand->order) }}" />
                            @error('order')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                        </div>
